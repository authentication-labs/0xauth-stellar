...
export async function getPublicKeys(registration: PublicKeyCredential) {
  const contractSalt = hash(base64url.toBuffer(registration.id));

  const response = registration.response as AuthenticatorAttestationResponse;
...
function getPublicKeyObject(attestationObject: Buffer) {
...
export function convertEcdsaSignatureAsnToCompact(sig: Buffer) {
  ...
  if (bufToBigint(s) > (bufToBigint(q) - BigInt(1)) / BigInt(2)) {
    signature64 = Buffer.from([
      ...r,
      ...Buffer.from(bigintToBuf(bufToBigint(q) - bufToBigint(s)) as Buffer),
    ]);
  } else {
