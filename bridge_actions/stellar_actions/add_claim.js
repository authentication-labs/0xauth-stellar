exports.handler = async function (payload) {
    ...
    try {
                    ...
                    if (key === 'matchReasons' && Array.isArray(conditionRequest[key])) {
                        conditionRequest[key].forEach((reason) => {
                            if (reason.args) {
                                ...
                                user = reason.args[0];
                                topic = reason.args[1];
                                scheme = reason.args[2];
                                issuer = reason.args[3];
                                signature = reason.args[4];
                                dataFromEVM = reason.args[5];
                                uri = reason.args[6];
                                ...
                                console.log(`dataFromEVM: ${dataFromEVM}`);
                                console.log(`URI: ${uri}`);
        ...
        console.log("Waiting for 10 seconds before getting ID...");
        await delay(10000);
        ...
        console.log("Waiting for 5 seconds before sending the transaction...");
        await delay(5000);
        ...
        if (/^\d+$/.test(scheme)) {
            schemeBigInt = BigInt(scheme);
        } else {
            throw new Error(`Invalid input for BigInt conversion: ${scheme}`);
        }
        ...
        console.log('Sending addClaim transaction...');
